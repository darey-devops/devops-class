...
resource "aws_eks_node_group" "eks-demo-cluster-ng" {
  ...
  tags            = merge(var.default_tags, tomap({"Name" = "eks-demo-cluster-ng"}))
...
resource "null_resource" "add_custom_tags_to_asg" {
  ...
  provisioner "local-exec" {
    command = <<EOF
aws autoscaling create-or-update-tags --tags ResourceId=${local.asg_name},ResourceType=auto-scaling-group,Key="Name",Value="EKS-MANAGED-NODEGROUP-NODE",PropagateAtLaunch=true
aws autoscaling create-or-update-tags --tags ResourceId=${local.asg_name},ResourceType=auto-scaling-group,Key="Custodian-Scheduler-StopTime",Value="off",PropagateAtLaunch=true
EOF
  }
