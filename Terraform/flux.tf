      ...
      spec {
        ...
        container {
          ...
          args = [
            ...
            "--git-url=git@github.com:darey-devops/devops-class.git",
            "--git-branch=fluxcd-class",
            "--git-path=code",
            "--git-label=flux",
            "--git-email=tahirjavedbhutta@gmail.com",
            "--listen-metrics=:3031",
            "--git-poll-interval=6m",
            "--sync-interval=6m",
            "--git-ci-skip=true",
...
resource "null_resource" "wait_for_rollout" {
  triggers = {
    deployment_id = kubernetes_deployment.memcached-dep.id
  }

  provisioner "local-exec" {
    command = "kubectl rollout status deployment/memcached -n flux --timeout=300s"
  }

  depends_on = [kubernetes_deployment.memcached-dep]
}
